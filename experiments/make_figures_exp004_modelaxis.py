...
def main():
    ...
    for p in ARMS:
        print(f"{p:<18}" + "".join(f"{data[mid].get(p, float('nan')):.2f}".rjust(16) for mid, _ in present))

    figstyle.apply(seed=42)
    fig, ax = plt.subplots(figsize=(7.0, 4.8))
    xs = list(range(len(present)))
    for pol in ARMS:
        ys = [data[mid].get(pol) for mid, _ in present]
        if any(y is None for y in ys):
            continue
        ax.plot(xs, ys, marker="o", markersize=8, linewidth=1.8,
