...
| EXP-009 | agentic-frames | The compaction-policy ranking holds and AMPLIFIES on a real agent transcript: the agent drives search/read tool use, so blind truncation discards evidence it gathered (irrecoverably) while keep-the-raw recovers it | running | n=20 directional: reversible_hybrid **0.40** > externalize **0.30** > truncate **0.20** (hybrid doubles truncate). Oracle-FRAMES gap was 0.04; agentic gap 0.20 — the agentic setting amplifies ~5x. Bootstrap CI positive [+0.05,+0.40], McNemar p=0.125 at n=20; scale-up running. Closes the agentic-domain + oracle-realism gaps |
